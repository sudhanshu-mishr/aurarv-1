    ...
    <section id="isa" className="relative bg-[#020305]/85 py-28 border-t border-white/5 backdrop-blur-[2px]">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <LayoutGroup id="isa-table-motion-group">
          ...
          {/* View Mode 1: Full Matrix Tabular View with Smooth Framer Motion Reordering */}
          {viewMode === 'matrix' && (
            <div className="overflow-x-auto rounded-2xl border border-white/10 bg-[#060a12]/80 backdrop-blur-md shadow-2xl">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/10 bg-white/[0.02] text-xs font-mono uppercase tracking-wider text-zinc-400">
                    ...
                    <th className="py-4 px-6">Instruction</th>
                    <th className="py-4 px-6">Type</th>
                    <th className="py-4 px-6">Opcode / Funct</th>
                ...
                <tbody className="divide-y divide-white/5 text-xs font-mono relative">
                  <AnimatePresence initial={false} mode="popLayout">
                    {filteredInstructions.length > 0 ? (
                      filteredInstructions.flatMap((inst) => {
                        const isExpanded = expandedInst === inst.name;
                        const rows = [
                          /* Main Instruction Summary Row with Framer Motion layout */
                          <motion.tr
                            key={`row-${inst.name}`}
                            layout
                            ...
                            transition={{
                              layout: { type: 'spring', stiffness: 480, damping: 34, mass: 0.8 },
          ...
          {/* View Mode 2: Interactive Animated Feed & Live Inspector */}
          {viewMode === 'animated-list' && (
            <motion.div
              layout
            ...
            >
              ...
              {/* Right Column: Live Bitfield & RTL Telemetry Inspector for Selected Instruction (col-span-7) */}
              <div className="lg:col-span-7 flex flex-col gap-6">
                <div className="rounded-2xl border border-white/10 bg-[#060a13] p-7 backdrop-blur-xl shadow-2xl">
                  {/* Header */}
                  ...
                  {/* 32-bit Machine Instruction Encoding Breakdown */}
                  <div className="mt-6">
                    <div className="flex items-center justify-between mb-2">
