...
const SHAPE_MODES: Record<string, number> = { mixed: 0, squares: 1, circles: 2, triangles: 3 };
const MAX_DPR = 2;
...
const parseColor = (value: any, fallback: string): [number, number, number] => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const glowSize = (size: [number, number]): [number, number] => [
        ...
        const applyMask = () => {
          ...
          applySettings();
          const settings = settingsRef.current;
          if (!settings.text.trim() || !(document as any).fonts?.load) return;
